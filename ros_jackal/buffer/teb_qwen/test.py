...
# 获取已存在的world编号
existing_worlds = set(df['World'].unique())

# 找出缺失的world (0-299)
all_worlds = set(range(300))
missing_worlds = sorted(all_worlds - existing_worlds)

print(f"已有world数量: {len(existing_worlds)}")
print(f"缺失world数量: {len(missing_worlds)}")
print(f"缺失的world: {missing_worlds}")