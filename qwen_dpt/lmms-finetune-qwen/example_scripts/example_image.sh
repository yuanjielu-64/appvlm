...
MODEL_ID=qwen2.5-vl-7b-instruct                                   # model id; pick on by running `python supported_models.py`
TRAIN_DATA_PATH=/scratch/bwang25/app_data/ddp_heurstic/splits_200k/chunk_000.json  # path to the training data json file
IMAGE_FOLDER=/scratch/bwang25/app_data/ddp_heurstic                     # path to the image root folder; if provided, the image paths in the json should be relative
...
RUN_ID=${MODEL_ID}_lora-${USE_LORA}_0_200k_ddp    # a custom run id that determines the checkpoint folder and wandb run name
...
torchrun $DISTRIBUTED_ARGS train.py \
    ...
    --output_dir /scratch/bwang25/checkpoints/$RUN_ID \
    --report_to wandb \
    --run_name $RUN_ID \
