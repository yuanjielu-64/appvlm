...
MODULE_KEYWORDS: Dict[str, Dict[str, List]] = {
    ...
    "qwen2.5-vl-regression": {
        "vision_encoder": ["base_model.visual.patch_embed", "base_model.visual.rotary_pos_emb", "base_model.visual.blocks"],
        "vision_projector": ["base_model.visual.merger"],
        "llm": ["base_model.model"],
        # LoRA 需要的 keys（不带 base_model. 前缀，因为在 model.base_model 上搜索）
        "llm_lora": ["model"]
    },
    "llama-3.2-vision": {
...
# qwen2.5-vl-regression (custom for navigation) -------------
register_model(
    model_id="qwen2.5-vl-regression",
    model_family_id="qwen2.5-vl-regression",
    model_hf_path="Qwen/Qwen2.5-VL-3B-Instruct"  # Use 3B for 10GB GPU
)
